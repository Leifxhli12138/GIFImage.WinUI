...
namespace GIFImage.WinUI.Core
{
    public partial class GifDecoder
    {
        ...
        protected Frame currentImage;
        protected Frame lastImage;
        ...
        protected List<Frame> frames = new();
        ...
        public Frame GetImage() => GetFrame(0);
        ...
        public Frame GetFrame(int n) =>
            (n >= 0 && n < frameCount) ? frames[n] : null;

        public Size GetFrameSize() => new Size(width, height);

        [Obsolete($"Please use {nameof(ReadAsync)} instead of the current method for more convenient asynchronous decoding and playback", true)]
        ...
        {
            var data = currentImage.Bitmap.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.WriteOnly, PixelFormat.Format32bppArgb);
            ...
            if (lastDispose > 0)
            {
                ...
                if (lastImage != null)
                {
                    var lastData = lastImage.Bitmap.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
                    byte[] prev = new byte[stride * height];
                    System.Runtime.InteropServices.Marshal.Copy(lastData.Scan0, prev, 0, prev.Length);
                    lastImage.Bitmap.UnlockBits(lastData);
                    Array.Copy(prev, 0, dest, 0, prev.Length);

                    if (lastDispose == 2)
                    {
                        using (Graphics g = Graphics.FromImage(currentImage.Bitmap))
                        {
            ...
            currentImage.Bitmap.UnlockBits(data);
        }
        ...
        protected void ReadImage()
        {
            ...
            frameCount++;
            currentImage = new();
            //currentImage.Bitmap = new Bitmap(width, height, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
            //SetPixels();
        ...
        protected byte[] GetPixels()
        {
            ...
            if (lastDispose > 0)
            {
                ...
                if (lastImage != null)
                {
                    Array.Copy(lastImage.MetaData, 0, dest, 0, lastImage.MetaData.Length);
                    if (lastDispose == 2)
                    {
                        Array.Clear(currentImage.MetaData, 0, currentImage.MetaData.Length);
                    }
                }
            }
            ...
            for (int i = 0; i < ih; i++)
            {
                ...
                if (line < height)
                {
                    ...
                    while (dx < dlim)
                    {
                        ...
                        if (color != 0)
                        {
                            dest[dx + 0] = (byte)(color & 0xff);         // B
                            dest[dx + 1] = (byte)((color >> 8) & 0xff);  // G
                            dest[dx + 2] = (byte)((color >> 16) & 0xff); // R
                            dest[dx + 3] = (byte)((color >> 24) & 0xff); // A
                        }
                        dx += 4;
                    }
                }
